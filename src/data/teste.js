...
const testee = details.find(item => item.currencyCode == 'ZAR');
console.log(testee.countries);
...
currencies.map((item, index) => {
    ...
    country.map((x, index) => {
        ...
        if (x.currency_code == item.code) {
            teste.push(x.country);
        };
    });
    full.push({ currencyCode: item.code, currencyName: item.name, countries: teste });
});
